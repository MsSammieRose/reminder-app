...
const { MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, MYSQL_DB, PORT } = process.env;
...
app.get("/api/v1/admin/reminder/list", (req, res) =>
  useHandler(new ListRemindersHandler(reminderDao), req, res)
);
app.post("/api/v1/admin/reminder/create", (req, res) =>
  useHandler(new CreateReminderHandler(reminderDao), req, res)
);
